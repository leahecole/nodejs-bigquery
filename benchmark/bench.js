...
if (process.argv.length < 3) {
    throw new Error('need query file; ' +
        "usage: '".concat(process.argv[0], " ").concat(process.argv[1], " <queries.json>'"));
}
console.log('query,rows,cols,first_byte,total');
var queryJson = fs.readFileSync(process.argv[2], 'utf8');
var queries = JSON.parse(queryJson);
var client = new src_1.BigQuery();
Promise.all(queries.map(function (query) {
    return doQuery(query).catch(console.error);
})).catch(console.error);
function doQuery(queryTxt) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            return [2 /*return*/, new Promise(function (resolve, reject) {
                    ...
                    var query = { query: queryTxt, useLegacySql: false };
                    var stream = client
                        ...
                        if (numRows === 0) {
                            ...
                            var error = new Error("query \"".concat(queryTxt, "\": ") +
                                "wrong number of columns, want ".concat(numCols, " got ").concat(receivedCols));
                            reject(error);
                        }
                        numRows++;
                    })
                        ...
                        console.log("\"".concat(queryTxt, "\",").concat(numRows, ",").concat(numCols, ",").concat(timeFirstByteMilli / 1000, ",").concat(timeTotalMilli / 1000));
                        resolve();
                    });
                })];
        });
